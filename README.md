...
## 7. รูปสินค้า (Master_Products) — เก็บไฟล์จริงใน Google Drive ไม่ต้องผูกบัตรเครดิต
ฟีเจอร์อัปโหลดรูปสินค้า (`uploadProductImage`/`deleteProductImage` ใน `Code.gs`) เก็บไฟล์จริงไว้ที่ **Google Drive ของบัญชีที่ deploy สคริปต์** (ไม่ใช่ Firebase Storage — ของเดิมต้องอัปเกรดโปรเจกต์เป็น Blaze plan ผูกบัตรเครดิตก่อนถึงจะเปิดใช้ Storage ได้ แม้จะอยู่ในโควตาฟรีก็ตาม จึงเปลี่ยนมาใช้ Drive แทน ไม่ต้องผูกบัตรเลย) และไม่ใช่ Firestore field ตรงๆ (จะชนขีดจำกัด 1MB/เอกสารและทำให้ทุกหน้าที่โหลดสินค้าทั้งหมดอืด)

ระบบสร้างโฟลเดอร์ Drive ชื่อ **"SYY Shop - Product Images"** ให้อัตโนมัติในการอัปโหลดครั้งแรก (จำ folder id ไว้ใน Script Properties คีย์ `PRODUCT_IMAGE_DRIVE_FOLDER_ID`) ตั้งชื่อไฟล์ในโฟลเดอร์ตาม `{รหัสสินค้า}.jpg` เสมอ อัปโหลดรูปใหม่ทับของเดิม = ลบไฟล์เก่าทิ้งแล้วสร้างไฟล์ใหม่ (ตั้งสิทธิ์แชร์แบบ "ทุกคนที่มีลิงก์ดูได้" ให้อัตโนมัติทุกไฟล์)

Drive ไม่มี "path คงที่" ให้คำนวณ URL จาก `productCode` ตรงๆ ได้เหมือน Storage เดิม (ต้องรู้ Drive file id) จึงเก็บ URL รูปไว้ใน Firestore collection แยกต่างหาก **`Product_Images`** (1 doc ต่อสินค้า 1 ชิ้น คีย์เอกสาร = productCode เก็บแค่ URL/file id) — ไม่ยุ่งกับเอกสาร `Master_Products` เลย เพื่อกันไม่ให้การแก้ไขสินค้าปกติ (ซึ่ง `saveMasterData` เขียนทับทั้งเอกสารทุกครั้งแบบไม่มี updateMask) ไปลบข้อมูลรูปทิ้งโดยไม่ตั้งใจ

**ไม่ต้องตั้งค่าอะไรเพิ่มก่อนใช้งาน** — สิทธิ์ `https://www.googleapis.com/auth/drive` มีประกาศอยู่ใน `appsscript.json` อยู่แล้ว (ผู้ใช้ authorize ตอนเปิดเว็บแอปครั้งแรกตามปกติ) โฟลเดอร์และสิทธิ์แชร์ไฟล์ถูกสร้าง/ตั้งให้อัตโนมัติทั้งหมดจากโค้ด
